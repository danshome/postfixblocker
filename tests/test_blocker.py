...
def test_write_map_files():
    entries = [blocker.BlockEntry(pattern="a@example.com", is_regex=False),
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with open(os.path.join(tmp, "blocked_recipients"), "r", encoding="utf-8") as f:
            content = f.read()
            assert "a@example.com" in content
        with open(os.path.join(tmp, "blocked_recipients.pcre"), "r", encoding="utf-8") as f:
            content = f.read()
