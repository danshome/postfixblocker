...
test('add, list, and delete entries', async ({ page }) => {
  ...
  await page
    ...
    .fill('e2e1@example.com');
  await page.getByRole('button', { name: 'Add', exact: true }).click();
  await expect(
    body.locator('tr', { hasText: /e2e1@example\.com/ }),
  ).toBeVisible();
  ...
  const rowByPattern = body.locator('tr', { hasText: /@e2e\.com\b/ });
  await expect(rowByPattern).toBeVisible({ timeout: 30_000 });
  await expect(rowByPattern.locator('td').nth(2)).toHaveText('Yes', {
    timeout: 30_000,
  });
  ...
  await page
    .getByPlaceholder('paste emails or regex (one per line)')
    .fill('bulk1@example.com\nbulk2@example.com');
  await page.getByRole('button', { name: 'Add', exact: true }).click();
  // Wait for both bulk rows to appear in the table body
  await expect(
    body.locator('tr', { hasText: /bulk1@example\.com/ }),
  ).toBeVisible({ timeout: 15_000 });
  await expect(
    body.locator('tr', { hasText: /bulk2@example\.com/ }),
  ).toBeVisible({ timeout: 15_000 });
